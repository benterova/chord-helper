    ...
    return (
        <div
            ...
            style={{
                ...
                margin: '2px 4px', // Add margin for spacing
                fontSize: '10px',
                cursor: 'grab',
                borderRadius: '12px', // Pill shape
                background: isActive
                    ? 'linear-gradient(180deg, #d0f0ff 0%, #a0d8ef 50%, #70c0e0 51%, #e0f7ff 100%)' // Active Liquid Blue
                    : 'linear-gradient(180deg, #ffffff 0%, #f0f0f0 50%, #e0e0e0 51%, #ffffff 100%)', // Default Glass/Silver
                border: isActive ? '1px solid #50a0d0' : '1px solid #bbb',
                boxShadow: isActive
                    ? '0 0 5px rgba(0,180,255,0.6), inset 0 1px 0 rgba(255,255,255,0.8)'
                    : '0 1px 2px rgba(0,0,0,0.1), inset 0 1px 0 rgba(255,255,255,0.8)',
                color: isActive ? '#004466' : '#555',
                textShadow: isActive ? '0 0 2px rgba(255,255,255,0.8)' : 'none',
                height: '24px', // Fixed small height
                flexShrink: 0
            }}
            className="aero-chord-pill" // Use new class if needed, but inline styles cover most
            {...attributes}
            {...listeners}
        >
            <span style={{ fontWeight: '700' }}>{chord.chordName}</span>
            <span style={{ fontSize: '9px', opacity: 0.8 }}>{chord.roman}</span>
        </div>
    );
};
export const CircleOfFifths: React.FC = () => {
    ...
    // --- Interactive State ---
    // --- Interactive State ---
    const [customSequence, setCustomSequence] = useState<{ id: string, chord: Chord }[]>([]);
    ...
    useEffect(() => {
        ...
        const lastItem = customSequence[customSequence.length - 1];
        if (!lastItem) return;
        const lastChord = lastItem.chord;
    ...
    const handleChordClick = (chord: Chord | undefined) => {
        ...
        if (isRecording) {
            setCustomSequence(prev => [...prev, { id: crypto.randomUUID(), chord }]);
        }
    };
    ...
    const handlePlaySequence = async () => {
        ...
        const sequenceToPlay = useSmartVoicing
            ? applySmartVoicing(customSequence.map(i => i.chord)).map(vc => ({ notes: vc.notes, duration: 1.0 }))
    ...
    const handleDownload = () => {
        ...
        const sequenceToExport = useSmartVoicing
            ? applySmartVoicing(customSequence.map(i => i.chord))
            : customSequence.map(i => ({ chord: i.chord, notes: getChordMidiNotes(i.chord) }));
        ...
        <div className="circle-section" ref={containerRef} onMouseMove={handleMouseMove} style={{ display: 'flex', flexDirection: 'row', height: '100%', background: 'transparent', fontFamily: '"Segoe UI", sans-serif', position: 'relative', overflow: 'hidden' }}>
            ...
            <div className="aero-sidebar" style={{
                ...
                <div style={{ flex: 1, overflowY: 'auto', padding: '5px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    ...
                    >
                        <SortableContext
                            items={customSequence.map(item => item.id)}
                    ...
                    <svg viewBox={`0 0 ${size} ${size}`} style={{ width: '100%', height: '100%', maxHeight: '420px', filter: 'drop-shadow(0 15px 25px rgba(0,160,255,0.3))' }}>
                        ...
                        <g transform={`translate(${center}, ${center}) rotate(-90)`}> {/* Rotate so C is top */}
                            ...
                            {CIRCLE_OF_FIFTHS.map((note, index) => {
                                ...
                                const isPlayingChord = playbackIndex !== null && customSequence[playbackIndex]?.chord.root === note;
